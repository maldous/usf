...
ADDITIONAL_TARGETS = {
    "default": [ROOT / "compose/compose.yaml"],
    "dev": [ROOT / "compose/compose.dev.generated.yaml"],
    "test": [ROOT / "compose/compose.test.generated.yaml"],
    "staging": [ROOT / "compose/compose.staging.generated.yaml"],
    "production": [ROOT / "compose/compose.production.generated.yaml"],
}
GENERATED_HEADER = [
...
def write_target(environment: str, catalogue: dict[str, Any]) -> Path:
    ...
    rendered = render_compose(catalogue, environment)
    target.write_text(rendered, encoding="utf-8")
    for additional_target in ADDITIONAL_TARGETS.get(environment, []):
        additional_target.parent.mkdir(parents=True, exist_ok=True)
        additional_target.write_text(rendered, encoding="utf-8")
    return target


def target_paths(environment: str) -> list[Path]:
    return [TARGETS[environment], *ADDITIONAL_TARGETS.get(environment, [])]
...
def main() -> int:
    ...
    if args.check:
        ...
        for target_name in targets:
            ...
            for path in target_paths(target_name):
