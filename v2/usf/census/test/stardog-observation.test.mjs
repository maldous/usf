...
test('independent Stardog observation verifies signature, trust, repository, digests, validation and rollback', () => {
  ...
  const names = ['load', 'validate-authored', 'derive', 'validate-derived', 'integrity', 'contamination', 'verify-counts', 'commit'];
  const payload = {
    schemaVersion: 1,
    ...
    rollback: { ok: true, faultCount: 8, digestsUnchanged: true, faults: names.map((name) => ({ name, rollbackCount: 1, errorPhase: name })) },
  };
  const envelope = {
    payload,
  ...
  try {
    ...
    assert.equal(result.observation.rollbackFaultCount, 8);
