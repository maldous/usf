...
| `packages/proof/src/mock-provider-substrate-proof.ts` | new-with-rationale | runtime-proof-support | Profile-gated local Compose proof aggregates webhook-sink capture and WireMock bounded proof while retaining LocalStack and live-provider non-claims; it does not copy React runtime/application code and does not prove LocalStack, standalone mock OIDC service equivalence, notification delivery, or live provider compatibility. |
| `packages/proof/src/wiremock-composed-proof.ts` | new-with-rationale | runtime-proof-support | Profile-gated local Compose proof for configured WireMock HTTP mock semantics through the adapter-contained wiremock-captain SDK boundary; it records LocalStack, live provider compatibility, provider certification, API runtime binding, worker runtime binding, environment readiness, and USF-133 closure as non-claims. |
