...
A dispatch watchdog (tools/github-runner/watchdog-runner.sh, installed host-side with a 3-minute systemd timer) heals the observed listener failure mode where the runner service stays alive and GitHub still reports the runner online, but the listener stops dispatching after broker socket errors, disposed message tokens, or stale canceled job messages (observed on the run 29019768528 stall; a related stall also occurs during upstream GitHub Actions incidents, where restart does not help and the watchdog only records evidence). The watchdog never acts while a runner Worker process is live, requires two consecutive error-tail observations before restarting, applies a fifteen-minute restart cooldown, and appends machine-readable evidence to the host evidence log. Watchdog restarts are operational hygiene only and prove nothing about validation.
