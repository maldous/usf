...
| 9 | DB / RLS / migrations | schema, migration | PostgreSQL 16, versioned forward-only migrations, per-tenant RLS policies | partial | Static/bootstrap proof only | known | parity-db | `relational-storage-and-migrations-and-rls`; data-migration-backup-restore plan |
