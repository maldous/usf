...
Additional hostnames such as www, app, api, auth, telemetry, or health are not required until a later issue explicitly declares them with environment, purpose, protocol, proof command, owner, risk owner, and control owner.
...
The preferred non-product proof route is:

- /.well-known/usf-public-edge.json

The allowed alternate route is:

- /__proof/public-fqdn

The route is a proof endpoint, not product UI.
...
USF-263 must prove external DNS, TLS, HTTPS, and proof endpoint delivery. USF-264 must prove public route delivery and Faro or Sentry telemetry bootstrap through the narrow Playwright boundary. USF-265 must authorise the proof tag only after the public FQDN gates are merged and reconciled.
