...
USF-233 tightens the bounded local proof into an explicit zero-open-issue gate for the supported local synthetic scan scope. The proof fails unless the quality-gate status is OK, the unresolved issue count bucket is zero, and the security hotspot count bucket is zero for that synthetic project. This is not a repository-wide SonarQube readiness claim, policy administration claim, vulnerability clearance claim, or live-provider claim.
