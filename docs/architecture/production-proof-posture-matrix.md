...
This document defines the proof posture required before the implementation extraction directive can be issued and before later implementation or release gates can claim readiness. It creates no proof evidence, imports no runtime/application code, emits no generated report, and promotes no schema to `active`.
...
| Posture | providerMode | environment | Required proof level | Freshness requirement | Gate type | Required for implementation proof substrate | Required before production-live release | Notes |
|---|---|---|---|---|---|---|---|---|
...
For the authentication proof substrate:

- Live-external-provider proof is **not required before the implementation merge**.
- Production-live proof is **not required before the implementation merge**.
...
- External sandbox, production-shaped, live-external-provider, and production-live proof are **release-blocking**, not hermetic implementation-merge blockers.
