...
| Status              | Draft / USF-152 implementation evidence                                          |
...
| `capabilities/notify/src/index.ts` | source-derived-rewrite | React notification dispatch, preferences, suppression, audit, and transport behavior | Fresh tenant-scoped notification service with PDP, audit, safe rendering, job enqueue, idempotency, suppression, consent, and delivery evidence. |
...
| `packages/proof/src/notifications-messaging-proof.ts` | source-derived-rewrite | React notification runtime proof lineage | Fresh hermetic `notify-proof` for USF notification behaviour and no-live-provider boundary. |
...
| Templates, consent depth, unsubscribe, bounce, complaint, deliverability, live providers | deferred/lineage-only where not implemented in React | posture defined; deeper runtime tracked as deferred depth |
