...
**6.E.1 Narrow external-tooling exception — `.github/workflows/`.** `.github/workflows/` MAY exist **solely** to hold GitHub Actions workflow files for the authorised fail-closed spec-validator CI gate (`tools/validate-spec/`), because GitHub mandates that exact path and provides no alternative location for repository CI. This is a narrow, fixed-name external-tooling exception, of the same kind as the conventional fixed-name tool files in §9 (e.g. `LICENSE`, `Makefile`, lockfiles). It authorises **only** `.github/workflows/` and **only** for GitHub Actions workflow files; it MUST NOT be generalised to any other top-level directory, and `.github/` MUST NOT hold non-CI content. Workflow file names remain governed by §16 (verb-object) and §9 where applicable. Any further CI/automation directory requires its own explicit authorisation.
