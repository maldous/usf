...
| **Evidence basis** | Grounded in the historical repository `../react` (the *historical repository*). Citations to `../react/...` are references to existing historical paths. |
...
| 6 | **Historical `../react` source and semantic evidence** | *What was the prior intent / behaviour / operational knowledge?* | `../react` (external) |
| 7 | **Generated reports** | *What is the current summary of the above?* | `evidence/` (generated) |
...
**What it cannot decide.** It cannot decide *whether* a behaviour was actually exercised (that is proof, rank 4) or *whether* a behaviour is permitted by decision (that is an ADR, rank 2 — semantics define behaviour, ADRs constrain which behaviours may exist and how they may change). It cannot certify itself: a semantic claim of completeness is only valid when its facets cite real evidence. (Historically, a capability may remain `delivered-and-proven` *only* when `semanticCompleteness.status` is complete and every facet points at authoritative evidence; otherwise it **MUST** drop to `semantic-gap`.)

**How future AI MUST use it.** As the **primary input**. An agent **MUST** read the relevant semantic definitions before changing behaviour, UI, tests, or proofs, and **MUST** make implementation conform to them. An agent **MUST NOT** reconstruct intended behaviour from source code, generated reports, or the historical repository when a USF semantic definition exists.

**Conflict behaviour.** Semantics outrank all lower classes on questions of intended behaviour. A standing contradiction between semantics and an accepted ADR (rank 2) is **drift**, not a precedence win: both cannot stand, and the agent **MUST** stop and reconcile (§4.5). Semantics **MUST NOT** be edited to match what the code happens to do (§4.1, §4.6).
...
**What it is.** The canon of normative architectural decisions under `docs/adr/`, each generated from semantic and proof evidence, recording *what was decided, the alternatives considered, the rationale, what it supersedes, and explicit AI-alignment constraints*. Historically: 74 accepted decisions (`v2-decision-catalog.json`) each paired with an evidence-backed lineage record (`v2-decision-lineage.json`: originating decisions, actions, evidence paths, commits).
...
**What it cannot decide.** It cannot, by itself, *define* product behaviour absent from semantics — an ADR **MUST** align to semantic definitions, not substitute for them. It cannot be **generic prose**: a decision that carries no machine-checkable consequence carries no governance force (historically a roadmap was rejected *because "a roadmap carries no governance force"*, V2-ADR-0053). It cannot stand untraced: a decision *"not Accepted or with no lineage"* is invalid (historically rejected by validator rule R13), and a lineage reference that does not resolve to a real decision, action, or evidence path is a finding.
...
**What it is.** The deterministic, read-only machine checks under `tools/` that enforce consistency across semantics, ADRs, proofs, references, Make/env/config, and implementation, and detect drift. Historically `tools/v2-readiness/` (~60 rules, R1–R62) plus the architecture-governance suite (`tools/architecture/`): *"a deterministic, read-only checker"* that *"fails closed: any contradiction is an error and exit 1,"* *"normalises no aliases,"* and *"writes no runtime file."*

**What it can decide.** Whether the artefacts are mutually consistent; whether a claim is **admissible** (e.g. that a "zero-gap" or "delivered-and-proven" assertion is permitted); whether a gate passes or fails; and whether the repository is in a state safe to act on. It decides **pass/fail**, fail closed on ambiguity, with explicit exit codes (historically: `0` pass, `1` finding, `2` bad input; `--strict` makes warnings fatal).
...
**Conflict behaviour.** Validators outrank proof, source, historical evidence, and reports: if the code or a report says "fine" but a validator finds a contradiction, the validator wins and the state is failing. Validators are **below** semantics and ADRs: a validator that disagrees with accepted semantics/ADRs is fixed to match them, not used to override them.

**Evolution.** When semantics or ADRs introduce a new class of behaviour, a validator rule **MUST** be added or extended to enforce it (historically the source-of-truth policy itself is enforced and its absence fails — R28-style). Validators **MUST** remain falsifiable: negative controls (deliberately broken fixtures) **MUST** demonstrate that the validators catch the violations they claim to catch.

### 2.4 Runtime proof evidence — rank 4

**What it is.** Executed, recorded, commit-pinned evidence under `evidence/` that exercises defined behaviour, graded on an explicit proof ladder and tagged with the provider class and substrate used. Historically: a seven-level ladder **L0 Discovery → L1 Executable → L2 Contract → L3 Behaviour → L4 Substrate (real local) → L5 Resilience → L6 Foundation**, with each record carrying `proofLevelClaimed` vs `proofLevelObserved`, provider-usage booleans (`mockProviderUsed`, `inMemoryProviderUsed`, `realLocalProviderUsed`, `externalSandboxProviderUsed`, `liveSubstrateUsed`), and a master index pinned to the current commit.
...
**What it is.** The extracted application, package, and service code under `apps/`, `packages/`, `services/`, and related roots that **implements** the semantic contracts. Historically a hexagonal API/BFF plus a web application and ~40 packages with enforced import boundaries.
...
### 2.6 Historical `../react` source and semantic evidence — rank 6

**What it is.** The complete prior body of work in `../react`: source, the historical semantic corpus, the validator and tooling, the decision and lineage records, runbooks, Make/env/config, tests, e2e harness, and generated proof evidence. It is **evidence and lineage**, captured at a frozen point (historically an immutable `v1-final` tag and a pinned freeze commit).

**What it can decide.** Nothing about USF's live behaviour. Its role is to let USF **recover intent**: prior behaviour, contracts, commands, Make/env/config semantics, tests, proofs, and operational knowledge. It is the **primary place to recover a missing intent** — because USF source (rank 5) **MUST NOT** be used to infer missing semantics, the historical repository is where absent intent is sourced from, then promoted into USF semantics and an ADR.

**What it cannot decide.** It **MUST NOT override accepted USF semantics once they exist.** *"Historical evidence and a proof reference … not the live semantic authority for new platform behaviour … reopened only for evidence correction"* (historical). Historical naming, transitional vocabulary, and branch identity **MUST NOT** be carried into USF (Charter §1.3, §4).

**How future AI MUST use it.** As a **reference to consult, not a template to copy**. An agent **MAY** read it to recover intent, commands, and operational knowledge, and **MUST** cite it as lineage when promoting recovered intent into USF semantics/ADRs. An agent **MUST NOT** copy historical artefacts into USF unexamined, and **MUST NOT** let historical behaviour override an accepted USF decision (§4.5).

**Conflict behaviour.** Historical evidence is overridden by all USF artefact classes (ranks 1–5). It outranks only generated reports. Where historical behaviour conflicts with an accepted USF ADR, the USF ADR wins and the historical behaviour is treated as superseded lineage (§4.5).

**Evolution.** It is **frozen**. What evolves is USF's *interpretation* of it into USF semantics and decisions. The historical repository itself is reopened only to correct evidence, never to extend USF behaviour.

### 2.7 Generated reports — rank 7 (lowest)
...
**How future AI MUST use it.** As a **convenience, never as the source of truth**. An agent **MUST NOT** treat a report as canonical; it **MUST** verify against the artefacts and re-generate the report from current state when in doubt. Historically the runtime-inventory roll-up is *"reported separately from formal proof readiness so runtime inventory closure cannot be mistaken for full migration proof"* — USF preserves this separation.
...
**Ruling.** The report (rank 7) is the lowest authority; **stale evidence voids the PASS**.
...
**Ruling.** Forbidden (Charter §6). Hermetic evidence **cannot** satisfy a live-provider claim; the live-provider floor (historically `liveProviderMinimumLevel: 4`) is not met by hermetic/in-memory/mock substrates.
**Action.** Relabel the evidence honestly by provider class and level; do not present it as live readiness. Production readiness **MUST fail rather than substitute mock responses for unavailable real providers.** The validators **MUST** catch a hermetic proof labelled as live (historically demonstrated by negative controls such as `in-memory-labelled-real-provider` and `fake-http-labelled-l4`).

### 4.5 Historical `../react` behaviour conflicts with an accepted USF ADR
**Ruling.** The USF ADR (rank 2) wins over historical evidence (rank 6).
**Action.** Treat the historical behaviour as superseded lineage; record the supersession in the ADR's lineage. Do not reopen the historical repository to override the USF decision — it is reopened only for evidence correction. If the historical behaviour reveals the USF decision is wrong, change the decision through a new ADR with coupled updates (§5), citing the historical evidence as rationale.

### 4.6 Implementation changes without a semantic update
...
5.1 A change that updates only the implementation, or only a report, is **incomplete** and the validators **MUST** fail closed (§4.6).
...
6.3 **Preserve lineage.** Keep traceability from semantics and decisions to their source and historical evidence. When you promote recovered intent from the historical repository, cite it as lineage. Never break or fabricate lineage to make a change look cleaner.

6.4 **Never infer missing semantics from implementation alone.** If a needed semantic definition is **absent**, do **not** invent it from USF source and do **not** invent it freely. Recover the intent from the historical repository (rank 6) where it exists, then **propose** the semantic definition together with its ADR and validator rule, and stop for that proposal to be accepted before implementing. Absent intent is a **stop condition**, not a guess.
