...
This matrix turns the first-pass V2 semantic-generation gap into a reviewable coverage plan. It inventories the 75 historical capability records and maps them to concrete USF semantic targets or explicit gaps so later semantic corpus expansion is driven by semantics rather than source resemblance.
...
|---:|---|---|---|---|---|---|
| 1 | `identity-access` | Tenant identity (record + FQDN) | first-slice | `semantic-contract.tenant-identity-record-and-fqdn; supports authentication-platform boundary` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=21; ops=yes; events=0; interactions=1; ui=0; proof=proof:domain-identity-matrix; proof:tenant-custom-domain-resolution |
| 2 | `identity-access` | User identity + tenant membership | first-slice | `semantic-contract.user-identity-and-tenant-membership; supports authentication-platform boundary` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=17; ops=yes; events=0; interactions=1; ui=0; proof=members unit + substrate tests |
...
| 7 | `identity-access` | RBAC (roles + permissions) | first-slice | `semantic-contract.rbac-roles-and-permissions; supports authentication-platform boundary` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=members tests |
...
| 13 | `authentication` | Platform login + session | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=7; ops=yes; events=0; interactions=0; ui=0; proof=auth-routes substrate tests; proof:auth-settings |
| 14 | `authentication` | IdP brokering + OIDC provider management | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=3; ops=yes; events=0; interactions=0; ui=0; proof=proof:auth-idps; proof:auth-oidc-enterprise |
| 15 | `authentication` | Claim mapping + group/role mapping | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=6; ops=yes; events=0; interactions=0; ui=0; proof=proof:ui-semantic-claim-mapping (headless journey); oidc-mapping unit tests |
| 16 | `authentication` | Real IdP login simulation | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=not-applicable-final; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=blocked |
| 17 | `authentication` | MFA + session policy + lockout | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=2; ops=yes; events=0; interactions=0; ui=0; proof=proof:auth-settings; test:platform-api:unit-safe; test:frontend:run |
...
| 61 | `authentication` | OIDC discovery / issuer / JWKS validation | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=proof:auth-oidc-enterprise |
| 62 | `authentication` | OIDC test connection + callback display | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=proof:auth-oidc-enterprise |
| 63 | `authentication` | OIDC login simulation | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=not-applicable-final; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=blocked |
| 64 | `identity-access` | Tenant host identity resolution | first-slice | `semantic-contract.tenant-host-identity-resolution; supports authentication-platform boundary` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=proof:domain-identity-matrix; proof:tenant-custom-domain-resolution |
...
| 67 | `authentication` | Tenant custom-domain auth callback | first-slice | `semantic-contract.authentication-platform` | USF-66 closes first-slice gaps; USF-58 authors missing child instances; USF-64 records per-file use policy. | status=delivered-and-proven; routes=0; ops=yes; events=0; interactions=0; ui=0; proof=proof:tenant-custom-domain-auth-origin |
...
These are the exact manifest-backed source rows selected by this matrix for the authentication platform/login first slice. Each maps to a concrete semantic target; rows are not permission to import runtime code.
...
- USF-74 must turn the first-slice source rows into a per-domain sub-manifest or an equivalent exact row-membership artefact.
- USF-64 must record source-use policy per first-slice row; `preserve`, `refactor`, or `replace` in the baseline manifest is not by itself permission to copy or adapt code.
