...
| `packages/proof/src/proof-review-public-proof.ts` | new-with-rationale | USF-290 human acceptance gate, USF-293 proof cockpit evidence model, live public proof-route availability requirement, ADR 0015 operator-authenticated edge semantics, and the existing proof package command pattern | Adds a fail-closed public HTTPS proof for the human acknowledgement surface at `/proof`. Its default mode verifies that the live composed Caddy edge is reachable over HTTPS while the operator-authenticated `/proof` gate remains closed with the committed 503 default and does not leak review controls or inline screenshot evidence. Its explicit open-review mode can verify the authenticated operator HTML surface with runtime credentials supplied outside the repository. It does not claim product UI readiness, Staging readiness, Production readiness, deployment readiness, live-provider readiness, SOC readiness, ISO certification, enterprise production readiness, browser E2E readiness, or full product readiness. |
