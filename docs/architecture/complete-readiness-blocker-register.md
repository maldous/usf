...
1. USF-101 (root): carrier decided (ADR-0006, signed annotated Git tag) and signer decided (ADR-0007, repository CI attestation identity, registered in the trust root). The only remaining step is operational: enable the proof-anchor CI workflow (`docs/runbooks/proof-anchor-publication.md`) and let the first post-merge CI run publish and verify a fresh signed anchor. Until that run occurs, USF-59, USF-73, and USF-99 cannot claim fresh proof.
