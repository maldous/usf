...
| Evidence and artifact sweep | `corepack pnpm repo:validate`, `corepack pnpm evidence-invalidation:validate`, `corepack pnpm evidence-reuse:validate`, and `corepack pnpm proof-cockpit:projection-repin:check` | Generated reports cannot replace raw evidence or validator output. |
