...
| Scope statement | The exact implementation scope. | Names every implementation slice, its non-goals, and any per-slice pre-file gate. For the current V2 migration, the directive must cover all slices rather than narrowing to one capability domain. | "Port the old app", "start with one domain", or any broad unbounded scope without per-slice gates. |
