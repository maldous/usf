...
The gate also enforces distinct per-rule planted-defect coverage. Every required validator rule from `USF-TEST-READINESS-001` through `USF-TEST-READINESS-111` must have at least one fixture whose `expectedRule` uniquely names that rule. Duplicating another valid planted defect or changing a fixture so it expects a different rule cannot satisfy coverage for the missing rule, even when the total fixture count remains unchanged.
