# Full React Functional Parity Readiness Directive
...
USF-39 closed the **local dev/test bootstrap** scope only (PR #88, PR #89; runtime bootstrap marker `v2-bootstrap` intentionally held at `91e3aac`). Bootstrap closure is **not** full foundation dev readiness and **not** full functional parity with the historical `../react` stack.

This directive defines, before any further implementation resumes, the complete migration bar: every implementation-relevant historical `../react` capability, service, route, workflow, job, port, adapter, provider behaviour, validation rule, command, proof, and test-backed behaviour MUST be **accounted for** and assigned exactly one disposition (§4). No foundation-relevant behaviour, and no historical test or proof, may be silently omitted.
...
2.2 The historical `../react` repository is **rank-6 evidence and lineage** (Authority Model §2.6). It MUST NOT be copied, path-mirrored, or treated as live authority. Behaviour is recovered into USF semantics, not imitated from source.
...
The USF foundation has **no UI/UX runtime**. "Full React functional parity" does **NOT** mean copying or rebuilding the historical React UI.
...
The genuinely **new** lens this directive adds — absent from the substrate above — is the **foundation-vs-UI scope classification** and the **test/proof classification** (especially Playwright/browser E2E), recorded in `react-parity-scope-classification-matrix.md`.
...
- What historical `../react` functionality exists? — 36 packages, 2 apps (platform-api, react-enterprise-app), 1 dev service (mock-oidc), 12 foundation domains, ~15 provider integrations.
