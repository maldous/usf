# USF V2 Foundation-Completeness Audit

| | |
|---|---|
| Document type | Architecture / USF foundation-completeness audit ledger |
...
| Source lineage basis | USF's own self-defined source-import baseline, frozen at commit `a92d9734cf0f1f7a53f9093ce3bb3d2c02bfd767`, tag `v1-final` (rank-5 source lineage) |
| USF base state reviewed | `fabe47b8fc70d34b34d1fc05c39da998c74a6748` (origin/main) |

This document is the consolidated USF V2 foundation-completeness audit. It harvests USF's own recorded semantic, proof, source-use, build, environment, CI, operational, data, configuration, governance, hexagonal-architecture, and UI source lineage and maps each to a USF V2 current-state record, an explicit gap, an explicit exclusion, or an explicit deferral.

It creates no implementation code, implementation directory, product runtime, source import, proof evidence, generated report, schema activation, or implementation directive. It does not start USF-39. It treats USF's own recorded source lineage as rank-5 source evidence only, never as future live authority. It mirrors no recorded source path as a USF target path. Every source-lineage citation is a verbatim reference into USF's own source-import registry, not a USF target.

## 1. Audit method and authority discipline

The audit was produced by inspecting USF's own recorded source lineage read-only and reconciling against the committed USF corpus (`spec/instances/`, `spec/registries/`, `evidence/`, `docs/architecture/`, `tools/validate-spec/`). Findings are classified, not asserted as proof. Per the Authority Model:

- Source-lineage proof output, generated reports, CI status, and local stdout are not proof authority for USF.
- Hermetic/mock proof is never upgraded to live-external-provider; production-shaped is never upgraded to production-live.
- No source-lineage capability is treated as a USF semantic contract until a USF semantic instance and a source-use disposition exist.
- Stale evidence cannot back current readiness.

Numbers attributed to USF's own source lineage are observed/approximate and are recorded as lineage, not as USF counts. Numbers attributed to USF are taken from the committed corpus and the existing matrices.
...
## 3. USF-103 — Source-lineage audit-base inventory and freeze

The pinned audit base is already enforced by the validator (`tools/validate-spec/validate-spec.py`, `SOURCE_AUDIT_BASE`, rule `USF-IMPORT-013`): USF's own self-defined source-import baseline, commit `a92d9734cf0f1f7a53f9093ce3bb3d2c02bfd767`, tag `v1-final`, entry count `1673`. The committed baseline manifest `spec/registries/source-import-manifest.json` carries those 1673 rows; the authentication-slice sub-manifest carries 159 rows (rule `USF-IMPORT-012`). This audit adopts that frozen base; it does not re-pin.

Observed source-lineage asset classes (lineage citations held in USF's source-import registry; counts approximate):
...
| Generated reports | `docs/v2-foundation/usf-audit/*`, `reports/` | `generated-report` (118 rows) | Rank-6 lineage only |
| Semantic docs | `docs/v2-foundation/*.json` | `semantic-artefact` (14 rows) | Governance-critical schemas |
| Migrations | `apps/platform-api/src/db/migrations/*.sql` | `data-migration-artefact` (36 rows) | Safety-sensitive (section 9) |
| Config | `config/environments/*`, `env/` | `configuration-file` (144 rows) | Section 10 |
| Make/CI | `Makefile`, `make/*.mk`, `.github/workflows/*` | `configuration-file` | Section 8 build/CI |
| Compose substrate | `compose.yaml` (~54 services), `docker/`, `infra/` | `configuration-file` | Substrate lineage; not authorised for USF runtime |
| E2E journeys | `e2e/**`, `playwright.*.config.ts` | `e2e-journey` (13 rows) | Profile split internal/build/identity/discovery/external/prod |

Audit-base classification: `v2-source-use-only` / `no-gap` for the frozen inventory itself. Candidate manifest coverage additions worth a future source-disposition decision (do not block whole-platform scope): `make/*.mk` build rules, `tools/semgrep/*` security rules, and `services/mock-oidc/**` substrate. Environment secrets and runtime caches (`.password`, `.npmrc`, `*.rvf`, `*.db`) are `excluded-not-applicable` / `missing-environment-posture` and must remain exclusions, never imported.
...
| All remaining historical capabilities (configuration, entitlements-billing, data-platform, events-workflow, compute-runtime, observability-ops, search, storage, security-governance, developer-platform, support-admin, foundation) | ~58 capabilities | `v2-semantic-only` (semantic captured; proof deferred) | `missing-source-use-disposition` / `missing-proof-evidence` | Represented as complete-facet draft contracts (source-backed where manifest rows exist, otherwise coverage-complete from USF's own recorded semantic source lineage); per-domain source-use matrices and manifest import (USF-74) remain open |

Reconciliation of USF-only authority instances back to source lineage: the `ai-governance` (authority-order and proof-freshness-anchor stop conditions), `command.validate-spec-*`, and `environment.*` instances are new USF-only authority (no source-lineage equivalent) and are correctly marked as such — they encode USF governance, not historical behaviour.

Distinction preserved: semantic capture (a USF instance exists) is separate from proof freshness and proof breadth. Authentication current-commit freshness is anchored by `proof-anchor-fabe47b`; non-authentication proof remains missing or lineage-only. No contract is inferred from source-lineage code; every proof-substrate mapping routes through the authentication-slice source-use disposition matrix (zero direct runtime-copy permission).
...
Recorded source-lineage proof ladder: L0 Discovery → L1 Executable → L2 Contract → L3 Behaviour → L4 Substrate(real-local) → L5 Resilience → L6 Foundation, with `proofLevelClaimed` vs `proofLevelObserved`, provider-usage booleans, and `liveProviderMinimumLevel: 4`. This maps 1:1 to the USF proof-level vocabulary enforced by the validator (`PROOF_LEVEL_ORDER`: `discovery-proven`…`foundation-proven`) and the USF rules `USF-PROOF-001/002`, `USF-EVIDENCE-006/007`, `USF-ANCHOR-004/005/006`.

Proof-equivalence taxonomy (proven / demonstrated / stale / missing / publication-blocked):

| Source-lineage proof domain | Recorded observed level / provider | USF state | Classification | Gap type |
|---|---|---|---|---|
| Authentication / domain-identity (`apps/platform-api/scripts/auth-settings-runtime-proof.ts`, `domain-identity-matrix-runtime-proof.ts`) | L3 behaviour, hermetic/contract | current USF authentication proof freshness is carried by `proof-anchor-fabe47b`; committed JSON remains historical | `v2-equivalent-current` | `no-gap` for authentication freshness; broader identity proof still requires per-slice proof |
...
| UI/Playwright/a11y journeys (`e2e/**`) | L3, hermetic UI | deferred until UI implementation scope | `deferred-post-v2` | `missing-proof-evidence` |

Lineage vs fresh-required:

- Treat as lineage (cite-only): all recorded L2–L3 contract/behaviour proofs and L4–L6 local/sandbox proofs in USF's source lineage. They are valid design input; they are not USF current readiness.
...
Evidence in USF's own source lineage that the foundation was materially hexagonal: explicit `ports/` and `adapters/` directories under `apps/platform-api/src/`; usecases depending on ports, not adapters; dependency-direction enforcement via `.dependency-cruiser.cjs`, `eslint.config.mjs`, and `knip.json`; TypeScript path aliases forcing public-export-only imports; a contracts/domain/adapter/runtime package split under `packages/`; and a UI layer importing only contract/design-system/i18n packages.
...
V2 must define UI contracts (ports/adapters/access-gating/accessibility/customisation) WITHOUT defining concrete UI implementation (concrete UI remains an AI-customisation deliverable). UI boundaries recorded in USF's own source lineage: permission gating (`RequirePermission`), session state (`use-session`), route guards (`_authenticated`), a11y (axe/Playwright), i18n runtime, BFF-only data access, branding/theming via a design system.
...
| Area | Source lineage | USF state | Classification | Gap type |
|---|---|---|---|---|
| Build entrypoint | `Makefile`, `make/*.mk`, `make all` | `target-implementation-topology-plan.md` records topology | `v2-source-use-only` | `missing-build-toolchain-decision` |
...
| Package graph | `packages/*` (~40) | topology plan lists allowed roots only | `v2-semantic-only` | `missing-build-toolchain-decision` |
| Environments | dev/test/staging/prod (`config/environments/*`) | `environment.hermetic`, `environment.production-shaped` instances; `multi-environment-proof-posture-closure.md` | `v2-semantic-only` | `missing-environment-posture` |
...
| Compose substrate | `compose.yaml` (~54 services), `docker/`, `infra/` | not authorised; topology plan keeps service/infra roots blocked | `v2-source-use-only` | `missing-environment-posture` |
| CI gates | `.github/workflows/*` (quality, architecture, CodeQL) | USF has `.github/workflows/validate-spec.yml` fail-closed gate + advisory validator | `v2-equivalent-current` (spec gate) / `v2-source-use-only` (product gates) | `no-gap` / `missing-validator-coverage` |
| Recorded readiness rules R1–R62 | `tools/v2-readiness/` | mapped by `usf-readiness-rule-coverage-matrix.md`; USF enforces its own rule set | `v2-semantic-only` | `missing-validator-coverage` (partial) |
...
| Migrations / RLS / tenant isolation | `apps/platform-api/src/db/migrations/*.sql` | `data.identity-schema` lineage; `data-migration-backup-restore-proof-slice-plan.md` | `v2-source-use-only` | `missing-semantic-contract` (safety-sensitive) |
| Backup / restore / PITR / retention / legal-hold | `scripts/backup/*`, `pitr-restore-drill-*` | planning docs only | `v2-source-use-only` | `missing-proof-evidence` |
| Object storage (tenant prefixes, signed URLs, quota) | `apps/platform-api/src/.../tenant-storage*` | deferred gap contract; `storage-data-governance-assurance-slice-plan.md` | `v2-semantic-only` | `missing-semantic-contract` |
| Configuration / secrets / provider settings | `config/environments/*`, OpenBao, `USF_PROVIDER_MODE` | `configuration.provider-mode-selector` (non-secret proof-substrate scope) | `v2-semantic-only` | `missing-semantic-contract` |
...
What USF's own recorded source lineage fully proved: contract/behaviour and local/sandbox substrate/resilience/foundation proofs across the platform. What USF has captured: the full semantic-contract corpus (64 complete-facet drafts, 1 deprecated, 2 non-applicable) plus governance, command, environment, and machine-readable bootstrap-mapping instances. What USF has proven fresh: the authentication proof-substrate freshness anchor for current main at hermetic-mock/hermetic/behaviour-proven. What is deferred/excluded: staging, production, live-external-provider, deployment, and production-live proof (ADR 0009 dev/test boundary), plus deprecated/not-applicable capabilities.
