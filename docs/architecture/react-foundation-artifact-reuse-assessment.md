# React Foundation Artefact Reuse Assessment

| | |
|---|---|
| **Document type** | Architecture / historical artefact reuse assessment |
...
| **Historical evidence basis** | `../react/docs/v2-foundation/**`, `../react/docs/evidence/**`, `../react/tools/v2-readiness/**`, `../react/package.json`, `../react/Makefile`, and `../react/make/**` |
| **USF repository state inspected** | `main` at `e0ec1368b33087ab5767aae4bb927f0850738773` |
| **React repository state inspected** | `../react` at `a92d9734cf0f1f7a53f9093ce3bb3d2c02bfd767` |

## Purpose

This assessment records how the historical React foundation artefacts can accelerate the remaining non-implementation prerequisites before USF-39. It treats the React repository as rank-6 historical source and semantic evidence only.
...
| React artefact or family | Observed shape | Primary classification | USF reuse posture |
|---|---:|---|---|
...
| `usf-audit/proof-evidence-index.json` | 173 proof-evidence records | evidence lineage; generated index | Reuse for proof lineage and coverage gap analysis; it is stale historical evidence for USF. |
| `usf-audit/proof-evidence/**` | per-proof JSON records | historical proof lineage | Reuse as source evidence and proof-command lineage only; not current runtime proof. |
...
| `tools/v2-readiness/tests/**` | golden, fixture, rule, import parity, proof evidence tests | validator inspiration | Reuse as negative-control design input for USF selftests. |
...
| Runtime proof scripts under `apps/platform-api/scripts/**` | proof runners tied to historical runtime package graph | proof lineage; implementation source candidate only under future directive | Evidence-only now; future adaptation requires explicit proof-substrate or implementation directive. |
...
| USF-68 report policy | generated readiness reports, formal graphs, assurance reports | `docs/architecture/generated-report-readiness-policy.md` | Existing policy is sufficient: generated reports are rank 7 and cannot close proof or semantic gaps. |
...
| USF-39 implementation extraction | source-use matrix, topology plan, semantic instances, import manifests, proof posture | Backlog only | Must not start. React artefacts are not an implementation directive. |

## Reuse Decisions

React artefacts that can be used directly as source evidence:
...
- proof evidence index and per-proof records as historical proof lineage only;
- readiness rules and tests as validator inspiration only.

React artefacts that require normalisation before they can become USF semantics:
...
React artefacts that require a new human/source-use decision before stronger use:
...
- any generated report proposed as more than a rank-7 summary.

React artefacts that must remain lower-authority:

- generated readiness summaries, assurance reports, formal graph reports, runtime inventories, and collection reports;
- stale historical proof records whose pinned commit is not the current USF commit;
- proof command outputs generated by the historical runtime.

## Readiness Implications

This audit does not reveal a safe way to advance USF-73 or USF-59 from the current repository state. The React proof scripts and Make/package commands are useful lineage and directive inputs, but they depend on runtime implementation, package graph, service topology, and proof harnesses that USF has not authorised.
...
2. expand semantic corpus instances for another domain using the React semantic artefacts, keeping implementation deferred;
...
- Do not execute historical proof scripts as USF proof.
- Do not treat historical React validator output as current USF proof.
