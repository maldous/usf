...
- Compatibility aliases that exist on current main, such as foundation,
  dev-ready, validate-evidence, and proof-review-repin.
- Future evidence validation and projection aliases only after their owning
  issues land, including evidence invalidation, evidence reuse, and
  proof-cockpit projection re-pin command surfaces.
