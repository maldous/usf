...
| Notification send/bulk posture | source-lineage notification/bulk/send lineage | partial | standard and proof posture | lineage-only | Notification guardrails represented; broad send quota rollout deferred. |
| Provider protection/backpressure | source-lineage provider/health/rate-limit lineage | partial | `adapters/guardrails/src/index.ts`, standard | rewrite-from-behaviour | Provider backpressure model represented without live provider enforcement. |
| Observability/security signals | source-lineage monitoring and request instrumentation lineage | in-scope | `apps/api/src/server.ts`, `packages/proof/src/rate-limits-abuse-controls-proof.ts` | rewrite-from-behaviour | Safe rate-limit and policy-denial signals through the local telemetry collector. |
| Audit/evidence | source-lineage audit and operational proof lineage | in-scope | `packages/core/src/index.ts`, `apps/api/src/server.ts` | rewrite-from-behaviour | Guardrail event taxonomy and value-free denial evidence. |
| Future API/ops guardrail surfaces | source-lineage admin/API posture evidence | partial | standard and USF-161 proof matrix | lineage-only | USF-161 proves a proof-local operator control-plane flow for policy draft, approval, publication, and value-free journal evidence. Public HTTP `/v1/guardrails/*` routes, UI, break-glass operation, and production operator readiness remain unclaimed. |
| Live WAF/edge/gateway/CDN/bot/fraud provider posture | source-lineage provider/adapters/compose lineage | deferred | standard and validator | lineage-only | Explicitly not implemented; exact future authority required before live integration. |
| UI/Playwright guardrail behaviours | source-lineage persona/UI/E2E rate-limit or abuse posture where present | foundation-behaviour-rewritten-from-ui-test | capability/API/proof tests | rewrite-from-behaviour | Foundation behaviours rewritten without Playwright; UI/UX remains separate. |

## Runtime File Disposition

| USF file | Treatment | Source-use basis | Notes |
| --- | --- | --- | --- |
| `docs/architecture/rate-limits-quotas-and-abuse-controls-standard.md` | source-derived-rewrite | source-lineage rate-limit/quota/abuse lineage plus USF-160 enterprise guardrail controls | Defines controlled guardrail semantics and deferred live depth. |
...
| `packages/core/src/index.ts` | source-derived-rewrite | source-lineage rate-limit/quota, audit, telemetry, API, provider posture lineage | Adds guardrail taxonomy, decisions, validation, safe messages, HTTP status mapping, and audit event types. |
| `packages/ports/src/index.ts` | source-derived-rewrite | source lineage/port lineage | Adds `GuardrailPort`. |
...
| `adapters/guardrails/src/index.ts` | source-derived-rewrite | source-lineage in-memory repository behaviour | Implements bounded in-memory local/dev/test policy store. |
| `apps/api/src/runtime.ts` | source-derived-rewrite | source-lineage runtime composition and route policy lineage | Seeds local jobs.create guardrail policy. |
| `apps/api/src/server.ts` | source-derived-rewrite | source-lineage API guard, retry-after, error-envelope, telemetry, audit lineage | Enforces jobs.create guardrail and safe 429 envelope. |
| `packages/contracts/src/api-surface.ts` | source-derived-rewrite | source-lineage API contract and route posture lineage | Documents concrete local guardrail posture and 429. |
...
| `packages/proof/src/rate-limits-abuse-controls-proof.ts` | source-derived-rewrite | source-lineage proof behaviour rewritten for USF | Hermetic guardrail proof plus USF-161 bounded synthetic distributed guardrails depth evidence. |
...
| `tests/capabilities/rate-limits-abuse-controls.test.ts` | source-derived-rewrite | source-lineage rate-limit/quota tests rewritten as foundation tests | Tests fail-closed, tenant-safe, idempotent guardrail behaviour. |
| `tests/apps/api-contracts.test.ts` | source-derived-rewrite | source-lineage API/route guard tests rewritten as foundation tests | Tests side-effecting route guard and safe telemetry/audit. |
