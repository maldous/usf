...
proof. It uses historical React artefacts only as background lineage where
...
| `package.json` | new-with-rationale | Existing proof command and verify wiring pattern | Adds proof:operator-access and wires it into verify without adding external provider dependencies or live-provider posture. |
| `tools/validate-enterprise/validate-enterprise.py` | lane-scoped-validator-extension | Lane 1 enterprise evidence validator pattern and USF-186 operator-access matrix validator | Adds USF-ENTERPRISE-019 to fail closed when the operator access proof command is missing, unwired from verify, missing required evidence markers, or overclaims readiness. |
