...
| Full platform semantic corpus | Closed at the semantic-contract level; source-use and proof remain open | The 67 semantic-contract targets named by the capability matrix all have instances: 64 complete-facet drafts (43 source-backed by manifest rows, 21 coverage-complete from the source-lineage semantic corpus; proof deferred), 1 deprecated, 2 non-applicable. The open work is per-domain source-use matrices and fresh proof, not missing semantic contracts. |
