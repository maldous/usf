...
| `identity-access` | 13 | Tenant identity, tenant host identity, user membership, and RBAC are source-backed drafts; remaining identity-access targets are explicit deferred gap contracts. | Replace deferred facets with source-backed semantics and source-use rows before implementation use. |
| `configuration` | 6 | Provider-mode selector is authored for the authentication slice; remaining configuration targets are explicit deferred gap contracts. | Author configuration-registry, branding, domain, secret-setting, and provider configuration semantics before implementation use. |
| `data-platform` | 6 | Identity schema lineage exists; remaining data-platform targets are explicit deferred gap contracts. | Author data semantics and source-use matrices; do not create migrations or storage runtime. |
...
| `events-workflow` | 4 | Authentication login workflow/event are authored for the current slice; broader event/workflow targets are explicit deferred gap contracts. | Author event/workflow contracts by capability and reconcile source-use rows. |
...
| `observability-ops` | 7 | Authentication audit signal exists; broader observability targets are explicit deferred or supersession gap contracts. | Author observability semantics and proof posture by signal family. |
