...
The public proof edge is a **composed** Caddy service (`external-caddy` in the generated staging Compose target), not a system/systemd Caddy. Its config is committed at `docker/caddy/Caddyfile` and mounted read-only at `/etc/caddy/Caddyfile` inside the container. There is no host `/etc/caddy` or systemd requirement: `make caddy-up` (`docker compose --profile gateway up -d external-caddy`) brings the edge up, and `make caddy-down` stops it. This composed edge is the accepted **public proof origin edge** (ADR 0015 + public-fqdn/public-proof-origin authority, USF-263/USF-264): its gateway ports are the only ports in the composed stack that bind the public interface — the generated staging Compose target publishes `0.0.0.0:80` and `0.0.0.0:443` for `external-caddy` (recorded in the service catalogue as `hostIp: 0.0.0.0`, `bindScope: public`, `internetExposureAllowed: true`) so the public read-only proof routes and the operator-authenticated `/proof` cockpit route are reachable over the public internet. Every other service — including `staging-proof-cockpit` and all operator/admin services — stays loopback-only behind this edge; only the `external-caddy` gateway ports are internet-exposed, and no operator/admin service is directly public. ACME/TLS material persists in the `caddy-data` named volume. Public reachability of the hermetic, synthetic proof content and the operator-authenticated cockpit is not a platform go-live and upgrades no readiness claim.
