...
Records the owner-ratified decision to expose the interactive staging proof cockpit (`apps/staging-proof-cockpit`) as a permanent, bounded staging operator fixture reachable at `https://<USF_PROOF_STAGING_FQDN>/proof` (default `https://1e100.network/proof`), served by a **composed** Caddy edge (`external-caddy`, from the committed `docker/caddy/Caddyfile`, with the staging and production FQDNs supplied as Makefile envs), gated by an operator credential (HTTP basic authentication) and reverse-proxied to a profile-gated `staging-proof-cockpit` Compose service reachable only over the compose network. It exists so the accountable operator can review the current proof state and record the single deliberate Accept or Reject decision described in the human-signoff runbook without any AI agent in the loop. The composed `external-caddy` edge is the accepted **PUBLIC proof origin edge**: its gateway ports bind public `0.0.0.0:80` and `0.0.0.0:443` on the staging FQDN edge so that the read-only public proof routes and the operator-authenticated `/proof` cockpit route are reachable over the public internet, consistent with the public-fqdn / public-proof-origin authority (USF-263 / USF-264). The decision preserves every existing platform non-claim: public reachability of hermetic synthetic proof content and an operator-authenticated proof cockpit is not a platform go-live and upgrades no readiness claim.
...
USF permits, as a bounded staging operator proof fixture, exposure of the interactive staging proof cockpit at `https://<USF_PROOF_STAGING_FQDN>/proof` (default `https://1e100.network/proof`). The exposure MUST satisfy all of the following. The public proof edge is a **composed** Caddy service (`external-caddy`) whose config is the committed `docker/caddy/Caddyfile`, mounted read-only into the container; there MUST be no host `/etc/caddy` file and no system/systemd Caddy requirement (clone-and-go). The two site addresses MUST be parameterised by env (`{$USF_PROOF_STAGING_FQDN}`, `{$USF_PROOF_PROD_FQDN}`), supplied as Makefile envs. The composed edge is the shared, always-on **public proof origin edge**: its gateway ports are the only ports in the composed stack that bind the public interface — `0.0.0.0:80` and `0.0.0.0:443` on the staging FQDN edge (recorded in the service catalogue as `hostIp: 0.0.0.0`, `bindScope: public`, `internetExposureAllowed: true`) — so the edge serves the public read-only proof routes (`/.well-known/usf-public-edge.json`, `/__proof/public-route`, and the no-store/security headers) to any internet client, independently of any operator gate. Every other service in the composed stack — including `staging-proof-cockpit` and all operator/admin/control-plane services — MUST stay loopback-only behind this edge; only the `external-caddy` gateway ports are internet-exposed. This accepted public exposure is the narrow, explicit exception recorded in the operator-access-gateway posture matrix (`acceptedPublicProofOriginEdge`) and the gateway/clickthrough substrate matrix; it authorises the caddy gateway ports only and does not relax the loopback-only default for any other service. The cockpit is realised as a profile-gated Compose service `staging-proof-cockpit` (profile `proof-cockpit`) generated for the staging Compose target, published with no host port at all — reachable only over the compose network as `staging-proof-cockpit:8080`. Public reachability is provided solely by the edge reverse proxy: a Caddy `proof_cockpit_handlers` snippet MUST be imported into the staging FQDN site only and MUST NOT be imported into the production FQDN (`{$USF_PROOF_PROD_FQDN}`) site. The `/proof` gate MUST require a valid operator credential via HTTP basic authentication before proxying any `/proof` request, and MUST preserve the established no-store and security headers. The gate MUST be a reload-managed include on a writable runtime mount whose committed default is CLOSED (responds 503, no unauthenticated fallback); the operator credential MUST live only in that runtime include (written at bring-up), never committed and never set in the `external-caddy` service environment. Cockpit write actions (the single Accept, the matching Reject, and QA actions) remain gated by the cockpit's own write policy: they are served only when `USF_PROOF_COCKPIT_ALLOW_WRITES` is `yes` and a non-empty `USF_PROOF_COCKPIT_REVIEW_SECRET` is present. Both MUST be supplied by the operator at bring-up (compose interpolation `${USF_PROOF_COCKPIT_ALLOW_WRITES:-}` / `${USF_PROOF_COCKPIT_REVIEW_SECRET:-}`); there MUST be no committed placeholder secret, so an unset secret leaves the cockpit read-only. Every write is CSRF-double-submit validated. The recorded actor is the configured accountable operator identity (`USF_PROOF_COCKPIT_REVIEW_ACTOR`), or the operator login forwarded by the edge. Acceptance is a single deliberate operator decision: one guarded Accept (POST `/proof/accept`) records, in one human action, an acceptance of every currently-open review item at its current evidence fingerprint and the final USF-290 human acceptance decision, and a matching Reject (POST `/proof/reject`) records the human-rejected decision. The single Accept is never auto-completed; it remains a deliberate human browser action requiring one confirmation checkbox ("I, the authenticated operator, accept the current proof state") in addition to the write guard and CSRF. The acceptance ledger persists to a durable Compose volume; promoting recorded acceptances into the committed `evidence/` corpus remains a manual operator reconciliation step and MUST NOT be performed by giving the running service repository write credentials. This decision does not claim that the live public deployment (DNS, TLS, and applying the composed edge on a public host) has been proven; that remains an operator step recorded in the runbook, and the platform non-claims below are preserved.
...
- `docs/architecture/gateway-clickthrough-access-substrate-matrix.json`
- `docs/architecture/public-fqdn-semantic-contract.md`
...
- The composed `external-caddy` edge is the accepted public proof origin edge: its gateway ports bind public `0.0.0.0:80` and `0.0.0.0:443` on the staging FQDN edge (`hostIp: 0.0.0.0`, `bindScope: public`, `internetExposureAllowed: true`). This public bind is authorised only for the caddy gateway ports; every other service, including `staging-proof-cockpit`, stays loopback-only behind the edge, and LAN exposure of any operator/admin service remains denied.
...
- Cockpit write actions (the single Accept, the matching Reject, and QA actions) are served only with `USF_PROOF_COCKPIT_ALLOW_WRITES=yes` plus a non-empty operator-supplied review secret (no committed placeholder), and every write is CSRF-double-submit validated.
- Acceptance is a single deliberate operator decision, never auto-completed: one guarded Accept (POST `/proof/accept`, requiring a single confirmation checkbox) records both the acceptance of every currently-open review item at its current evidence fingerprint and the final USF-290 human acceptance decision, and a matching Reject records the human-rejected decision; the machine surface never records the decision on its own.
...
- Do not extend the public `0.0.0.0` bind beyond the two `external-caddy` gateway ports; the `staging-proof-cockpit` and every other operator/admin/control-plane service MUST stay loopback-only behind the edge, and LAN exposure of any operator/admin service remains denied. Public reachability of any operator/admin surface is only ever via the operator-authenticated edge reverse proxy, never a direct public host port.
...
- The composed `external-caddy` edge (from `docker/caddy/Caddyfile`) becomes the shared, always-on public proof origin edge with FQDN envs, a persistent `caddy-data` volume, and a writable runtime mount for the reload-managed `/proof` gate include (committed default CLOSED). Its gateway ports bind public `0.0.0.0:80` and `0.0.0.0:443` on the staging FQDN edge; the generated staging Compose target (`compose/compose.staging.generated.yaml`) publishes `0.0.0.0:80` and `0.0.0.0:443` for `external-caddy`, and the dev/test edges remain host-portless (compose-network-only).
- The operator-access-gateway posture matrix and the gateway/clickthrough substrate matrix record the accepted public proof origin edge (`acceptedPublicProofOriginEdge`), and the operator-access matrix records the `/proof` cockpit route as an active, bounded, edge-authenticated operator fixture rather than `defined-only`. The `validate-enterprise` USF-ENTERPRISE-014 and USF-ENTERPRISE-020 rules honour this narrow accepted authority for the caddy gateway ports only and continue to fail closed for every other service and for public exposure asserted without a well-formed accepted-authority block.
...
- Agents MUST preserve the cockpit write guard (allow-writes plus review secret plus CSRF) and the single deliberate, non-auto-completed operator Accept/Reject decision (one confirmation checkbox); they MUST NOT reintroduce a machine-side auto-completion of acceptance.
